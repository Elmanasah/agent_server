...
export function getOrCreateAgent(sessionId, history = [], user = null) {
  if (!agentCache.has(sessionId)) {
    agentCache.set(
      sessionId,
      new Agent(buildSystemPrompt(user), history, user),
    );
  }
...
export class Agent {
  ...
  async _executeTool(name, args) {
    ...
    switch (name) {
      case "search_knowledge_base": {
        if (!this.userId)
          return {
            text: "No user context available for knowledge base search.",
          };
        const context = await retrieveContext(this.userId, args.query).catch(
          (err) => {
        ...
        return {
          text: context || "No relevant documents found in the knowledge base.",
        };
      }

      case "search_sessions": {
        if (!this.userId)
          return { text: "No user context available for session search." };
        const sessionContext = await searchSessions(
          this.userId,
          args.query,
        ).catch((err) => {
        ...
        return {
          text: sessionContext || "No relevant past conversations found.",
        };
      }
      ...
      case "read_session_transcript": {
        ...
        const sessionObj = await Session.findOne({
          where: { id: args.sessionId, userId: this.userId },
        });
        ...
        if (!messages.length)
          return { text: "No messages found for this session." };
      ...
      case "recall_facts": {
        ...
        return {
          text: memories.length
            ? JSON.stringify(memories.map((m) => `[${m.category}] ${m.fact}`))
            : "No facts matching this query.",
        };
      }
      ...
      case "manage_tasks": {
        ...
        if (args.action === "create") {
          ...
          const t = await Task.findOne({
            where: { id: args.taskId, userId: this.userId },
          });
      ...
      case "generate_image": {
        if (this.userId) {
          ...
          if (!imgUsage.allowed) {
            return {
              text: `Image limit reached (${imgUsage.limit}/period). Resets at ${imgUsage.reset_at}.`,
            };
          }
        }
  ...
  async sendMessage(
    ...
    if (this.userId) {
      ...
      if (usage.is_locked) {
        throw new Error(
          `Account locked: ${usage.lock_reason || "Contact support."}`,
        );
      }
      if (!usage.allowed) {
    ...
    while (loopCount < MAX_LOOPS) {
        ...
        functionCalls.map(async (part) => {
          ...
          if (toolOutput.canvas) {
            emit({
              type: "tool_result",
              tool: name,
              result: { canvas: toolOutput.canvas },
            });
          ...
          if (toolOutput.diagram) {
            emit({
              type: "tool_result",
              tool: name,
              result: { diagram: toolOutput.diagram },
            });
          ...
          if (outputText.length > MAX_OUTPUT_LENGTH) {
            outputText =
              outputText.substring(0, MAX_OUTPUT_LENGTH) +
      ...
      if (textParts.length) {
        ...
        if (intermediateText.trim())
          emit({ type: "token", text: intermediateText });
      }
